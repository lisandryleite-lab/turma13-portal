...
function TagGrupo({ grupo, small }: { grupo: string; small?: boolean }) {
  ...
  return (
    <span style={{ background: cor+"18", color: cor, border:`1px solid ${cor}40`,
      borderRadius:6, padding: small?"1px 7px":"2px 10px", fontSize: small?11:12, fontWeight:600 }}>
      {grupo}
    </span>
  )
}
...
function CardServico({ label, pessoa, destaque }: { label:string; pessoa:{mat:number;nome:string}|null; destaque?:boolean }) {
  return (
    <div style={{ background: destaque?"var(--azul-profundo)":"#fff",
      border:`1.5px solid ${destaque?"var(--azul-profundo)":"var(--cinza-borda)"}`,
      borderRadius:12, padding:"16px", textAlign:"center", boxShadow:"var(--shadow-sm)" }}>
      <p style={{ fontSize:11, fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
        color: destaque?"rgba(255,255,255,0.7)":"var(--cinza-texto)", marginBottom:6 }}>{label}</p>
      {pessoa ? (
        <>
...
function NavBtn({ onClick, children, disabled }: { onClick:()=>void; children:React.ReactNode; disabled?:boolean }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      background: disabled?"#f0f4ff":"var(--azul-profundo)", color: disabled?"#c5cde0":"#fff",
      border:"none", borderRadius:8, padding:"6px 14px", fontSize:14, fontWeight:700,
...
export function EscalasClient({
  ...
  servicoAtual, todasSemanas,
  mesesCalendario, composicaoFaxina, membrosPlantao,
  ...
  async function salvarOverride(s: number) {
    ...
    await fetch("/api/escalas", {
      ...
      body: JSON.stringify({
        tipo: "servico", semana: s,
        p1: editP1 ? Number(editP1) : null,
        p3: editP3 ? Number(editP3) : null,
        p4: editP4 ? Number(editP4) : null,
      }),
    })
    setSaving(false)
  ...
  async function removerOverride(s: number) {
    ...
    // Salva com null para limpar o override — backend vai deletar o registro
    await fetch("/api/escalas", {
      method: "DELETE",
      headers: { "Content-Type":"application/json" },
      body: JSON.stringify({ tipo: "servico", semana: s }),
    })
    router.refresh()
  }
  ...
  const isMinhaFaxina = (g: GrupoFaxina|null) => g ? composicaoFaxina[g].some(p=>p.mat===minhaMatricula) : false
        ...
        <div>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
            <h2 style={{ fontSize:14, fontWeight:700, color:"var(--azul-profundo)", letterSpacing:"0.06em", textTransform:"uppercase" }}>
              ...
              {semanaVis.isOverride && (
                <span style={{ fontSize:10, background:"var(--dourado)", color:"#fff", borderRadius:20,
                  padding:"2px 8px", marginLeft:8, fontWeight:700 }}>editada</span>
              )}
            </h2>
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              <NavBtn onClick={()=>setSemanaServicoIdx(i=>Math.max(0,i-1))} disabled={semanaServicoIdx===0}>‹</NavBtn>
              <span style={{ fontSize:12, color:"var(--cinza-texto)", minWidth:80, textAlign:"center" }}>
                {semanaServicoIdx+1}/{semanas.length}
              </span>
              <NavBtn onClick={()=>setSemanaServicoIdx(i=>Math.min(semanas.length-1,i+1))} disabled={semanaServicoIdx===semanas.length-1}>›</NavBtn>
            </div>
          </div>

          {/* Linha da semana visível */}
          {editandoSemana===semanaVis.semana ? (
            <div style={{ background:"#f8faff", border:"1.5px solid var(--azul-medio)", borderRadius:12, padding:16, marginBottom:16 }}>
              <p style={{ fontSize:13, fontWeight:700, color:"var(--azul-profundo)", marginBottom:12 }}>
                Editar semana {semanaVis.semana}
              </p>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:12 }}>
                {([
                  {label:"P1 — Pessoal",    val:editP1, set:setEditP1},
                  {label:"P3 — Operações",  val:editP3, set:setEditP3},
                  {label:"P4 — Logística",  val:editP4, set:setEditP4},
                ]).map(({label,val,set}) => (
                  <div key={label}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>salvarOverride(semanaVis.semana)} disabled={saving}
                  style={{ background:"var(--azul-profundo)", color:"#fff", border:"none", borderRadius:7,
                    padding:"7px 18px", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                  {saving?"Salvando...":"Salvar"}
                </button>
                {semanaVis.isOverride && (
                  <button onClick={()=>removerOverride(semanaVis.semana)}
                    style={{ background:"#fee2e2", color:"#b91c1c", border:"none", borderRadius:7,
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                <thead>
                  <tr style={{ background:"var(--azul-claro)" }}>
                    {["P1 — Pessoal","P3 — Operações","P4 — Logística"].map(h=>(
                      <th key={h} style={{ padding:"10px 14px", textAlign:"left", fontWeight:700, color:"var(--azul-profundo)", fontSize:12 }}>{h}</th>
                    ))}
                    {isAdmin && <th style={{ width:80 }}/>}
                  </tr>
                </thead>
                <tbody>
                  <tr style={{ borderTop:"1px solid var(--cinza-borda)", background:"var(--creme)" }}>
                    {[semanaVis.p1,semanaVis.p3,semanaVis.p4].map((p,i)=>(
                      <td key={i} style={{ padding:"12px 14px", fontWeight:p?.mat===minhaMatricula?700:400 }}>
                        {p?.nome ?? "—"}
                        {p?.mat===minhaMatricula && <span style={{ fontSize:11, color:"var(--azul-medio)", marginLeft:6 }}>← você</span>}
                      </td>
                    ))}
                    {isAdmin && (
                      <td style={{ padding:"12px 14px" }}>
                        <button onClick={()=>{
                          setEditandoSemana(semanaVis.semana)
                          setEditP1(semanaVis.p1?String(semanaVis.p1.mat):"")
                          setEditP3(semanaVis.p3?String(semanaVis.p3.mat):"")
                          setEditP4(semanaVis.p4?String(semanaVis.p4.mat):"")
                        }} style={{ fontSize:11, color:"var(--azul-medio)", background:"#f0f4ff",
                          border:"none", borderRadius:5, padding:"3px 10px", cursor:"pointer" }}>Editar</button>
                      </td>
                    )}
                  </tr>
                </tbody>
          ...
          {/* Mini lista de semanas */}
          <details style={{ marginTop:8 }}>
            <summary style={{ fontSize:12, color:"var(--cinza-texto)", cursor:"pointer", userSelect:"none", marginBottom:8 }}>
              Ver todas as semanas ({semanas.length})
            </summary>
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                <thead>
                  <tr style={{ background:"var(--azul-profundo)", color:"#fff" }}>
                    <th style={{ padding:"8px 12px", textAlign:"left", fontSize:11 }}>Semana</th>
                    <th style={{ padding:"8px 12px", textAlign:"left", fontSize:11 }}>P1</th>
                    <th style={{ padding:"8px 12px", textAlign:"left", fontSize:11 }}>P3</th>
                    <th style={{ padding:"8px 12px", textAlign:"left", fontSize:11 }}>P4</th>
                ...
                <tbody>
                  {semanas.map((s,i)=>{
                    ...
                    return (
                      <tr key={s.semana} style={{ borderTop:"1px solid var(--cinza-borda)",
                        background: atual?"var(--azul-claro)":eu?"#fffbf0":"#fff",
                        fontWeight: atual||eu?600:400 }}>
                        <td style={{ padding:"8px 12px", color:"var(--azul-profundo)" }}>
                          {s.semana}{atual?" ✦":""}
                          {s.isOverride&&<span style={{ fontSize:9, background:"var(--dourado)", color:"#fff", borderRadius:3, padding:"1px 4px", marginLeft:4 }}>ed</span>}
                        </td>
                        ...
                        {isAdmin && (
                          <td style={{ padding:"8px 12px" }}>
                            <button onClick={()=>{
                              setSemanaServicoIdx(i)
                              setEditandoSemana(s.semana)
                              setEditP1(s.p1?String(s.p1.mat):"")
                              setEditP3(s.p3?String(s.p3.mat):"")
                              setEditP4(s.p4?String(s.p4.mat):"")
                            }} style={{ fontSize:10, color:"var(--azul-medio)", background:"#f0f4ff",
                              border:"none", borderRadius:4, padding:"2px 7px", cursor:"pointer" }}>Editar</button>
                          </td>
                        )}
                      </tr>
                    )
        ...
        <div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:12, marginBottom:28 }}>
            {(Object.keys(composicaoFaxina) as GrupoFaxina[]).map(g => {
              ...
              return (
                <div key={g} style={{ background:euSou?"var(--azul-claro)":"#fff",
                  border:`1.5px solid ${euSou?"var(--azul-medio)":"var(--cinza-borda)"}`, borderRadius:12, padding:14 }}>
                  ...
                  <ul style={{ listStyle:"none", padding:0, margin:0, fontSize:12, color:"var(--grafite)" }}>
                    {composicaoFaxina[g].map(p=>(
                      <li key={p.mat} style={{ padding:"2px 0", fontWeight:p.mat===minhaMatricula?700:400 }}>{p.mat} {p.nome}</li>
                    ))}
                  </ul>
                </div>
              )
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              ...
              <span style={{ fontSize:12, color:"var(--cinza-texto)", minWidth:60, textAlign:"center" }}>
                {mesFaxinaIdx+1}/{mesesCalendario.length}
              </span>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {/* Plantão de hoje — apenas mês atual */}
        ...
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:12, marginBottom:28 }}>
            {(["GOLF","HOTEL","INDIA","JULIETT","KILO","LIMA","MIKE","NOVEMBER"] as GrupoPlantao[]).map(g=>{
              ...
              return (
                <div key={g} style={{ background:temEu?"var(--azul-claro)":"#fff",
                  border:`1.5px solid ${temEu?"var(--azul-medio)":"var(--cinza-borda)"}`, borderRadius:12, padding:14 }}>
          ...
          <h2 style={{ fontSize:14, fontWeight:700, color:"var(--azul-profundo)", marginBottom:12, letterSpacing:"0.06em", textTransform:"uppercase" }}>
            Adjuntos da Turma 13
          </h2>
          {plantaoDias.filter(p=>p.adjuntoMat).length===0 && (
            <p style={{ color:"var(--cinza-texto)", fontSize:13, marginBottom:16 }}>Nenhum dado inserido ainda.</p>
          )}
          <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:24 }}>
            {plantaoDias.filter(p=>p.adjuntoMat).map(p=>{
              ...
              return (
                <div key={p.id} style={{ background:eu?"var(--azul-claro)":"#fff",
          ...
          {isAdmin && (
            <div>
              <button onClick={()=>setShowPlantaoForm(!showPlantaoForm)} style={{ background:"var(--azul-profundo)", color:"#fff", border:"none", borderRadius:8, padding:"9px 18px", fontSize:13, fontWeight:600, cursor:"pointer" }}>
              ...
              {showPlantaoForm && (
                <div style={{ marginTop:16, background:"var(--azul-claro)", borderRadius:12, padding:20 }}>
                  <p style={{ fontSize:12, color:"var(--cinza-texto)", marginBottom:12 }}>
                    Informe as datas em que alunos da Turma 13 são adjuntos da 2ª CIA.
                  </p>
                  {plantaoEntradas.map((e,i)=>(
                    <div key={i} style={{ display:"flex", gap:8, marginBottom:8, flexWrap:"wrap" }}>
                      <input type="date" value={e.data} onChange={ev=>{const n=[...plantaoEntradas];n[i]={...n[i],data:ev.target.value};setPlantaoEntradas(n)}}
                        style={{ border:"1px solid var(--cinza-borda)", borderRadius:6, padding:"6px 10px", fontSize:13 }} />
                      <select value={e.adjuntoMat} onChange={ev=>{const n=[...plantaoEntradas];n[i]={...n[i],adjuntoMat:ev.target.value};setPlantaoEntradas(n)}}
                        style={{ border:"1px solid var(--cinza-borda)", borderRadius:6, padding:"6px 10px", fontSize:13 }}>
                        <option value="">— Adjunto —</option>
                        {alunos.map(a=><option key={a.matricula} value={a.matricula}>{a.matricula} {a.nomeGuerra}</option>)}
                      </select>
                      {plantaoEntradas.length>1&&<button onClick={()=>setPlantaoEntradas(plantaoEntradas.filter((_,j)=>j!==i))}
                  ...
                  <div style={{ display:"flex", gap:8, marginTop:8 }}>
                    <button onClick={()=>setPlantaoEntradas([...plantaoEntradas,{data:"",adjuntoMat:""}])}
                      style={{ background:"#e0e7ff", color:"var(--azul-profundo)", border:"none", borderRadius:6, padding:"7px 14px", fontSize:12, cursor:"pointer" }}>+ Linha</button>
        ...
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:28 }}>
            {Array.from(funcoesPorData.entries()).map(([dataKey,funcs])=>{
              ...
              return (
                <div key={dataKey} style={{ background:euTenho?"var(--azul-claro)":"#fff",
                  border:`1.5px solid ${euTenho?"var(--azul-medio)":"var(--cinza-borda)"}`, borderRadius:12, padding:"12px 16px" }}>
                  <p style={{ fontWeight:700, fontSize:13, color:"var(--azul-profundo)", marginBottom:8 }}>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                    {funcs.map(f=>(
                      <div key={f.id} style={{ background:f.matricula===minhaMatricula?"var(--azul-profundo)":"var(--creme)",
                        color:f.matricula===minhaMatricula?"#fff":"var(--grafite)", borderRadius:8, padding:"6px 12px", fontSize:12,
                        display:"flex", alignItems:"center", gap:8 }}>
                        <span style={{ fontWeight:600 }}>{f.funcao}</span>
                        <span>{nomesPorMat[f.matricula]??`Mat.${f.matricula}`}</span>
                        {isAdmin&&<button onClick={()=>deletarFuncao(f.id)}
                          style={{ background:"none", border:"none", cursor:"pointer",
                            color:f.matricula===minhaMatricula?"rgba(255,255,255,0.7)":"var(--cinza-texto)", fontSize:13, padding:0, lineHeight:1 }}>×</button>}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {isAdmin && (
            <div>
              <button onClick={()=>setShowFuncaoForm(!showFuncaoForm)} style={{ background:"var(--azul-profundo)", color:"#fff", border:"none", borderRadius:8, padding:"9px 18px", fontSize:13, fontWeight:600, cursor:"pointer" }}>
              ...
              {showFuncaoForm && (
                <div style={{ marginTop:16, background:"var(--azul-claro)", borderRadius:12, padding:20 }}>
                  {funcaoEntradas.map((e,i)=>(
                    <div key={i} style={{ display:"flex", gap:8, marginBottom:8, flexWrap:"wrap" }}>
                      <input type="date" value={e.data} onChange={ev=>{const n=[...funcaoEntradas];n[i]={...n[i],data:ev.target.value};setFuncaoEntradas(n)}}
                        style={{ border:"1px solid var(--cinza-borda)", borderRadius:6, padding:"6px 10px", fontSize:13 }} />
                      <select value={e.funcao} onChange={ev=>{const n=[...funcaoEntradas];n[i]={...n[i],funcao:ev.target.value};setFuncaoEntradas(n)}}
                        style={{ border:"1px solid var(--cinza-borda)", borderRadius:6, padding:"6px 10px", fontSize:13 }}>
                        {FUNCOES_DESTAQUE.map(f=><option key={f} value={f}>{f}</option>)}
                      </select>
                      <select value={e.matricula} onChange={ev=>{const n=[...funcaoEntradas];n[i]={...n[i],matricula:ev.target.value};setFuncaoEntradas(n)}}
                        style={{ border:"1px solid var(--cinza-borda)", borderRadius:6, padding:"6px 10px", fontSize:13 }}>
                        <option value="">— Aluno —</option>
                        {alunos.map(a=><option key={a.matricula} value={a.matricula}>{a.matricula} {a.nomeGuerra}</option>)}
                      </select>
                      {funcaoEntradas.length>1&&<button onClick={()=>setFuncaoEntradas(funcaoEntradas.filter((_,j)=>j!==i))}
                        style={{ background:"#fee2e2", color:"#b91c1c", border:"none", borderRadius:6, padding:"4px 10px", cursor:"pointer", fontSize:12 }}>✕</button>}
                    </div>
                  ))}
                  <div style={{ display:"flex", gap:8, marginTop:8 }}>
