    ...
    <main style={{ maxWidth: 480, margin: "0 auto", padding: "32px 24px" }}>
      ...
      {sucesso ? (
        <div style={{ marginTop: 16, padding: "20px 18px", borderRadius: 12, background: "#eef4ea", border: "1px solid var(--olive)", textAlign: "center" }}>
          ...
          <p style={{ color: "var(--ink)", marginTop: 8 }}>Senha alterada! Entre novamente com a nova senha.</p>
          {/* Encerra a sessão: o token em uso ainda diz que a senha é a inicial,
              e com ele o portal devolveria o aluno para esta mesma tela. */}
          <button
            type="button"
            onClick={() => signOut({ callbackUrl: "/login" })}
            style={{ marginTop: 14, padding: "10px 18px", borderRadius: 8, background: "var(--olive)", color: "var(--canvas)", border: "none", fontWeight: 600, fontSize: 14, cursor: "pointer" }}
          >
            Ir para o login
          </button>
        </div>
