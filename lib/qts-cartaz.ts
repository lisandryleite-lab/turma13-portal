// ── Cartaz do QTS — gerador de HTML para impressão ────────────────────────────
// Módulo puro, sem React e sem Prisma: usado pelo botão "Imprimir cartaz"
// (`app/(logado)/qts/qts-print.tsx`, que abre o HTML numa janela e chama
// window.print()) e pelo script `scripts/qts-cartaz-pdf.ts`, que renderiza o
// mesmo HTML em PDF via Chromium headless. Manter os dois consumindo daqui
// garante que o PDF sai idêntico ao que o portal imprime.

import type { QtsDados } from "./qts"

export type DisciplinaCartaz = {
  sigla: string
  nome: string
  cargaTotal: number
  cargaMinistrada: number
  status: string
}
...
export function corDisc(sigla: string) {
  let h = 0
...
export type OpcoesCartaz = {
  dados: QtsDados
  disciplinas: DisciplinaCartaz[]
  semana: number
  /**
   * Segunda página com o progresso do curso. Exige `disciplinas` com
   * cargaMinistrada real (vinda do banco) — sem isso o cartaz sairia com
   * números errados, então o script CLI a desliga quando roda sem DATABASE_URL.
   * Padrão: true.
   */
  acompanhamento?: boolean
  /** Injeta o window.print() no final do documento (uso no navegador). */
  autoPrint?: boolean
}

/** HTML completo e autocontido do cartaz da semana. */
export function cartazHtml({
  dados, disciplinas, semana, acompanhamento = true, autoPrint = false,
}: OpcoesCartaz): string {
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8">
      ...
      <body>
        ...
        ${legendaHtml}
        ${acompanhamento ? acompanhamentoHtml(disciplinas, semana) : ""}
...
function acompanhamentoHtml(disciplinas: DisciplinaCartaz[], semana: number): string {
  ...
  const barra = (d: DisciplinaCartaz) => {
